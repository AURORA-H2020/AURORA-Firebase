...
export const calculateCarbonEmissionsV2 = onDocumentWritten(
	{
		document: `${FirebaseConstants.collections.users.name}/{userId}/${FirebaseConstants.collections.users.consumptions.name}/{consumptionId}`,
		secrets: [recommenderApiToken, recommenderApiBaseUrl],
	},
	async (event) => {
		// Create secrets object from the secret values
		const secrets = {
...
async function calculateElectricityConsumptionEmissions(
	...
	// If the consumption is generated by an investment, set consumption to 0 and offset carbon Emission
	if (electricity.electricitySource === "pvInvestment") {
		return {
			carbonEmission:
				(context.consumption.value / householdSize) *
