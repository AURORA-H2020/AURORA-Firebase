		...
		await Promise.allSettled(
			recurringConsumptions.docs
				.map((recurringConsumptionDocument) => {
					...
					const recurringConsumption =
						recurringConsumptionDocument.data() as RecurringConsumption;
					...
					consumption.generatedByRecurringConsumptionId =
						recurringConsumptionDocument.id;
					// Retrieve the recurring consumptions collection reference
					const recurringConsumptionsCollection =
						recurringConsumptionDocument.ref.parent;
...
function getCurrentDate(
	...
	return new Date(
		date.toLocaleString("en-US", { timeZone: timeZone, hour12: false }),
	);
}
...
function frequencyMatches(
	...
	switch (recurringConsumption.frequency.unit) {
		...
		case "weekly": {
			...
			return (
				recurringConsumption.frequency.weekdays?.includes(weekday) === true
			);
		}
		case "monthly": {
			...
			return (
				currentDayOfMonth === numberOfDaysInMonth &&
				frequencyDayOfMonth > numberOfDaysInMonth
			);
		}
		default:
			return false;
			...
			return {
				...
				transportation: {
					dateOfTravel: Timestamp.fromDate(
						getCurrentDate({
							hours: transportation.hourOfTravel,
							minutes: transportation.minuteOfTravel,
						}),
					),
